# Strafing Mod
This mod brings strafing mechanics to Ratchet and Clank that try to match it's newer games in the serries, Ratchet and Clank: Up Your Arsenal and Ratchet: Deadlocked  No need to hold down a button, or anything like that!  It just works!

## Features
* Always Strafing! (Except when holding wrench, swingshot and a few more gadgets)
 * No need to press and hold any buttons to strafe like in Going Commando!
* Faster Camera!
 * The in game camera speeds (Slow, Medium and Fast) have all new values to play with!

## Game Support
The following Ratchet and Clank versions are supported:
| Game Name | PCSX2 PNACH File |
|-----------|------------------------|
| Ratchet and Clank (NTSC-U) | SCUS-97199_CE4933D0.pnach |
| Ratchet and Clank v1.00 (PAL) | SCES-50916_6A8F18B9.pnach |
| Ratchet and Clank v2.00 (PAL) | SCES-50916_76F724A3.pnach |
| Ratchet and Clank (NTSC-J) | SCPS-15037_56A35F77.pnach |

## Releases
Releases can be found in the [Releases](https://github.com/Metroynome/rac-cheats/tree/master/enable-strafing/release)
, or by clicking the following link:

### [Releases](https://github.com/Metroynome/rac-cheats/tree/master/enable-strafing/release)
